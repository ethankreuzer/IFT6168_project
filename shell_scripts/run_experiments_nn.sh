...
BASE_CMD="python /cim/ehoney/IFT6168_project/dcdi/main.py \
    --train \
    --data-path /cim/ehoney/IFT6168_project/dcdi/data/perfect/data_p10_e10_n10000_nn_struct \
    --num-vars 10 \
    --model DCDI-G \
    --intervention \
...
for i in {0..10}; do
    EXP_PATH="/cim/ehoney/IFT6168_project/experiments/ignoring_intervs_nn/dataset_${i_dataset}_regimes-ignored_${i}"
    mkdir -p "$EXP_PATH"
    if [ "$i" -eq 0 ]; then
